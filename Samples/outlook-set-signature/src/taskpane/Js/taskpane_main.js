...
function save_signature_settings() {
	...
	if (user_info_str) {
		...
		Office.context.roamingSettings.set('newMail', get_selection('new_mail'));
		Office.context.roamingSettings.set('reply', get_selection('reply'));
		Office.context.roamingSettings.set('forward', get_selection('forward'));
...
function get_selection(elementId) {
	const currentField = document.getElementById(elementId);
	return currentField.options[currentField.selectedIndex].value;
}
