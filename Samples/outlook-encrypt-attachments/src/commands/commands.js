...
// let totalOptionalAttendees = 0;
// let totalRequiredAttendees = 0;
// let totalDistributionLists = 0;

/** Initialize Office JS API library. */
Office.onReady();

/**
 * Run when an appointment is created or edited.
...
function onAppointmentComposeHandler(event) {
  ...
  Office.context.mailbox.item.start.getAsync((asyncResult) => {
    ...
    Office.context.mailbox.item.end.getAsync((asyncResult2) => {
      ...
      Office.context.mailbox.item.sessionData.setAsync("appointment_info", JSON.stringify(originalAppointmentDate), (asyncResult3) => {
...
/**
 * Runs when an email is created.
 * @param {Office.AsyncResult} result default: Office.AsyncResult
 */
function onMessageComposeHandler(event) {
  console.log("onMessageComposeHandler(): entered!");

  // Displays the InfoBar in the compose email. Reminds the user to open the task pane to display additional instructions and reference information.
  // NOTE: The actions parameter is only applicable to InsightMessage notifications.
  const options = { asyncContext: { callingEvent: event } };
  Office.context.mailbox.item.notificationMessages.addAsync(
    "showInfoBarForSampleInstructions",
    {
...
/**
 * Runs when an attendee is added or removed from an appointment.
 * @param {Office.AsyncResult} event default: Office.AsyncResult
 */
function onAppointmentAttendeesChangedHandler(event) {
  ...
  Office.context.mailbox.item.requiredAttendees.getAsync((asyncResult) => {
    ...
    if (asyncResult.status === Office.AsyncResultStatus.Succeeded) {
      ...
      // Get attendees except for the current user (which is only included in this array in classic Outlook on Windows).
      ...
      const apptRequiredAttendeesWithoutUser = apptRequiredAttendees.filter((attendee) => {return attendee.emailAddress !== Office.context.mailbox.userProfile.emailAddress;});
      totalRequiredAttendees = apptRequiredAttendeesWithoutUser.length;
      currentDistributionLists = apptRequiredAttendees.filter((attendee) => {return attendee.recipientType === "distributionList";});
      ...
      Office.context.mailbox.item.optionalAttendees.getAsync((asyncResult2) => {
        ...
        if (asyncResult2.status === Office.AsyncResultStatus.Succeeded) {
          ...
          const apptOptionalAttendees = asyncResult2.value;
          ...
          const apptOptionalAttendeesWithoutUser = apptOptionalAttendees.filter((attendee) => {return attendee.emailAddress !== Office.context.mailbox.userProfile.emailAddress;});
          totalOptionalAttendees = apptOptionalAttendeesWithoutUser.length;
          currentDistributionLists = apptOptionalAttendees.filter((attendee) => {return attendee.recipientType === "distributionList";});
          ...
          Office.context.mailbox.item.notificationMessages.replaceAsync(
            ...
            (asyncResult4) => {
              ...
              if (asyncResult4.status === Office.AsyncResultStatus.Succeeded) {  
                let anyExistingWarningMessages = false;
  
                Office.context.mailbox.item.notificationMessages.getAllAsync((asyncResult5) => {
...
function decryptAttachment(result) {
  ...
  const bytes = CryptoJS.AES.decrypt(result.asyncContext.encryptedData, secretKey);
...
/**
 * Runs when the user changes the date/time of an appointment.
...
function onAppointmentTimeChangedHandler(event) {
  ...
  Office.context.mailbox.item.sessionData.getAsync("appointment_info", (asyncResult) => {
    if (asyncResult.status === Office.AsyncResultStatus.Succeeded) {
      ...
      try {
        ...
        Office.context.mailbox.item.notificationMessages.getAllAsync((asyncResult2) => {
          ...
          const originalDateMessage = `Original date/time: Start = ${convertedToLocalStart.toLocaleDateString()} ${convertedToLocalStart.toLocaleTimeString()}; End = ${convertedToLocalEnd.toLocaleDateString()} ${convertedToLocalEnd.toLocaleTimeString()}`;
          Office.context.mailbox.item.notificationMessages.replaceAsync(
            "timeChanged",
            {
              type: Office.MailboxEnums.ItemNotificationMessageType.InformationalMessage,
